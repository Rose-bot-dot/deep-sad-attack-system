...
from .attack_mlp import AttackMLP
from .attack_mlp_autoencoder import AttackMLPAutoencoder
import os
import pandas as pd
...
def build_network(net_name, ae_net=None):
    ...
    elif net_name == 'attack_mlp':
        input_dim = get_attack_input_dim()
        net = AttackMLP(x_dim=input_dim, rep_dim=32)

    else:
        raise ValueError(f"未知网络: {net_name}")
...
def build_autoencoder(net_name):
    ...
    if net_name == 'thyroid_mlp':
        ...
        input_dim = get_attack_input_dim()
        ae_net = AttackMLPAutoencoder(x_dim=input_dim, rep_dim=32)
    return ae_net
